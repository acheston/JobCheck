...
export async function addPerson(personData) {
  ...
  if (personData.emailRecipients) {
    if (Array.isArray(personData.emailRecipients)) {
      emailRecipients = personData.emailRecipients.filter(email => email && String(email).trim().includes('@'));
  ...
  try {
    ...
    if (updates.emailRecipients !== undefined) {
      ...
      if (Array.isArray(updates.emailRecipients)) {
        arr = updates.emailRecipients.filter(email => email && String(email).trim().includes('@'));
      } else if (typeof updates.emailRecipients === 'string') {
        arr = updates.emailRecipients.split(',').map(e => e.trim()).filter(e => e && e.includes('@'));
      }
      updatedPerson.emailRecipients = arr;
    }
